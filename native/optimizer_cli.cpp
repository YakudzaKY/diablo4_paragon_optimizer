...
constexpr double RARE_ACCESS_PULL_FACTOR = 0.40;
constexpr double RARE_ACCESS_PULL_DECAY = 0.80;
constexpr int RARE_ACCESS_PULL_RADIUS = 5;
...
constexpr int LOCAL_PRIZE_ACCESS_RESCUE_MAX_NEW_NODES = 3;
constexpr int LOCAL_PRIZE_ACCESS_RESCUE_REMOVE_CANDIDATES = LOCAL_IMPROVEMENT_REMOVE_CANDIDATES;
constexpr int LOCAL_PRIZE_ACCESS_RESCUE_MAX_REMOVES = 3;
...
constexpr int FINAL_SINGLE_SWAP_MAX_PASSES = 4;
constexpr int FINAL_SINGLE_SWAP_REMOVE_CANDIDATES = 120;
constexpr int FINAL_SINGLE_SWAP_ADD_CANDIDATES = 96;
constexpr int DEFAULT_LOCAL_REPAIR_TOP_CANDIDATES = 4;
...
struct GlyphRouteTuning {
    ...
    double threshold_buffer = 0.0;
    double excess_factor = 0.02;
...
void read_glyph_route_tuning(const json& value, GlyphRouteTuning& tuning) {
    ...
    read_optional_tuning_double(value, "threshold_buffer", tuning.threshold_buffer, 0.0, 1000.0);
    read_optional_tuning_double(value, "excess_factor", tuning.excess_factor, 0.0, 1.0);
...
bool glyph_has_useful_threshold_scaling(const GlyphInfo& info, double scaling_weight) {
    return info.scaling_value_per_5 > 0.0 && scaling_weight > 0.0;
}

double glyph_threshold_hint_fill_target(
    const GlyphInfo& info,
    const WeightModel& weights,
    double available_stat,
    double scaling_weight
) {
    double target = info.requirement;
    if (glyph_has_useful_threshold_scaling(info, scaling_weight)) {
        target = std::max(target, available_stat);
    } else if (weights.glyph_route.threshold_buffer > 0.0) {
        target = std::max(target, info.requirement + weights.glyph_route.threshold_buffer);
    }
    return std::min(available_stat, target);
}
...
double glyph_threshold_hint_fill_target(
    ...
    double target = info.requirement;
    if (glyph_has_useful_threshold_scaling(info, scaling_weight)) {
        target = std::max(target, info.requirement * weights.glyph_route.fill_target);
    } else if (weights.glyph_route.threshold_buffer > 0.0) {
        target = std::max(target, info.requirement + weights.glyph_route.threshold_buffer);
    }
    return target;
...
std::map<std::string, GlyphRouteStatPressure> glyph_route_stat_pressures(
    ...
    for (int socket_index : graph.glyph_sockets) {
        for (int glyph_index = 0; glyph_index < static_cast<int>(glyphs.size()); ++glyph_index) {
            ...
            double target = glyph_threshold_hint_fill_target(info, weights, supply, scaling_weight);
            ...
            if (!info.threshold_stat.empty() && info.requirement > 0.0) {
                ...
                if (available_stat > 0.0) {
                    ...
                    if (activation_pool > 0.0 || (info.scaling_value_per_5 > 0.0 && scaling_weight > 0.0)) {
                        ...
                        matrix.expected_fill = glyph_threshold_hint_fill_target(info, weights, available_stat, scaling_weight);
...
bool is_prize_route_node(const GraphNode& node) {
    return node.type == "rare" || node.type == "legendary";
}

std::vector<double> rare_access_route_potentials(
    const Graph& graph,
    const std::vector<double>& base_scores,
    double low_value_threshold
) {
    std::vector<double> potentials(graph.nodes.size(), 0.0);
    if (graph.nodes.empty() || low_value_threshold <= 0.0) {
        return potentials;
    }

    for (int source : reachable_nodes(graph)) {
        const GraphNode& prize = graph.nodes[source];
        if (!is_prize_route_node(prize)) {
            continue;
        }
        double prize_value = std::max(base_scores[source], 0.0);
        if (prize_value <= 1e-12) {
            continue;
        }
        ...
        while (!queue.empty()) {
            auto [current, distance] = queue.front();
            queue.pop();
            if (distance >= RARE_ACCESS_PULL_RADIUS) {
                continue;
            }
            ...
            for (int neighbor : graph.adjacency[current]) {
                ...
                if (next_distance > RARE_ACCESS_PULL_RADIUS) {
                    continue;
                }
                seen[neighbor] = 1;
                queue.push({neighbor, next_distance});

                const GraphNode& access_node = graph.nodes[neighbor];
                if (access_node.cost <= 0) {
                    continue;
                }
                if (base_scores[neighbor] >= low_value_threshold) {
                    continue;
                }
                double decay = std::pow(RARE_ACCESS_PULL_DECAY, static_cast<double>(next_distance));
                potentials[neighbor] += prize_value * RARE_ACCESS_PULL_FACTOR * decay;
            }
        }
    }
...
std::vector<int> best_value_path_tree(const RouteInput& input, const std::vector<unsigned char>& selected) {
    struct PathState {
        int cost = 0;
        double gain = 0.0;
        int node = -1;
    ...
    struct ComparePathState {
        bool operator()(const PathState& left, const PathState& right) const {
            if (left.cost != right.cost) {
                return left.cost > right.cost;
            }
            if (std::abs(left.gain - right.gain) > 1e-12) {
    ...
    std::vector<int> best_cost(input.node_ids.size(), std::numeric_limits<int>::max());
    std::vector<double> best_gain(input.node_ids.size(), -std::numeric_limits<double>::infinity());
    std::priority_queue<PathState, std::vector<PathState>, ComparePathState> queue;
    for (size_t index = 0; index < selected.size(); ++index) {
        if (selected[index]) {
            previous[index] = -1;
            best_cost[index] = 0;
            best_gain[index] = 0.0;
            queue.push({0, 0.0, static_cast<int>(index)});
        }
    }
    while (!queue.empty()) {
        PathState state = queue.top();
        queue.pop();
        int current = state.node;
        if (state.cost != best_cost[current] || state.gain < best_gain[current] - 1e-12) {
            continue;
        }
        for (int neighbor : input.adjacency[current]) {
            int add_cost = selected[neighbor] ? 0 : input.costs[neighbor];
            double add_gain = selected[neighbor] ? 0.0 : input.scores[neighbor];
            int candidate_cost = state.cost + add_cost;
            double candidate_gain = state.gain + add_gain;
            bool better_cost = candidate_cost < best_cost[neighbor];
            bool better_gain = candidate_cost == best_cost[neighbor] &&
                candidate_gain > best_gain[neighbor] + 1e-12;
            if (better_cost || better_gain) {
                previous[neighbor] = current;
                best_cost[neighbor] = candidate_cost;
                best_gain[neighbor] = candidate_gain;
                queue.push({candidate_cost, candidate_gain, neighbor});
            }
        }
    }
    return previous;
...
RouteOutput run_greedy_route(const RouteInput& input, const std::vector<int>& target_pool) {
    ...
    while (points_used < input.points) {
        Candidate best;
        std::vector<int> previous = best_value_path_tree(input, output.selected);
...
RouteInput build_route_input(
    ...
    base_scores.reserve(size);
    for (int index = 0; index < size; ++index) {
        const GraphNode& node = graph.nodes[index];
        double base_score = route_node_score(node, weights, &route_bonuses, index);
        base_scores.push_back(base_score);
    }
    double score_reference = route_score_reference(base_scores);
    input.low_value_threshold = score_reference * 0.35;
    std::vector<double> access_potentials = rare_access_route_potentials(
        graph,
        base_scores,
        input.low_value_threshold
    );
    for (int index = 0; index < size; ++index) {
        const GraphNode& node = graph.nodes[index];
        input.node_ids.push_back(node.id);
        input.costs.push_back(node.cost);
        input.scores.push_back(base_scores[index] + access_potentials[index]);
...
json glyph_route_tuning_json(const GlyphRouteTuning& tuning) {
    return {
        ...
        {"threshold_buffer", round4(tuning.threshold_buffer)},
        {"excess_factor", round4(tuning.excess_factor)},
...
bool route_connected_selection(const Graph& graph, const std::vector<unsigned char>& selected) {
    if (graph.start_index < 0 || static_cast<size_t>(graph.start_index) >= selected.size() || !selected[graph.start_index]) {
        return false;
    }
    int expected = 0;
...
std::vector<int> shortest_unselected_path_from_route(
    const Graph& graph,
    const std::vector<unsigned char>& selected,
    int target,
    int max_new_nodes
) {
    std::vector<int> previous(graph.nodes.size(), -2);
    std::vector<int> new_count(graph.nodes.size(), 0);
    std::queue<int> queue;
    for (int index = 0; index < static_cast<int>(selected.size()); ++index) {
        if (selected[index]) {
            previous[index] = -1;
    ...
    while (!queue.empty()) {
        ...
        for (int neighbor : graph.adjacency[current]) {
            if (previous[neighbor] != -2 || selected[neighbor]) {
                continue;
            }
            int next_new_count = new_count[current] + 1;
            if (next_new_count > max_new_nodes) {
                continue;
            }
            previous[neighbor] = current;
            new_count[neighbor] = next_new_count;
            queue.push(neighbor);
        }
    }
...
std::vector<PrizeAccessPath> prize_access_paths(
    const Graph& graph,
    const std::vector<unsigned char>& selected,
    const WeightModel& weights,
    const std::unordered_map<int, double>& route_bonuses
) {
    std::vector<PrizeAccessPath> paths;
    for (int index = 0; index < static_cast<int>(graph.nodes.size()); ++index) {
        const GraphNode& node = graph.nodes[index];
        if (selected[index] || !is_prize_route_node(node)) {
            continue;
        }
        double prize_value = route_node_score(node, weights, &route_bonuses, index);
        if (prize_value <= 1e-9) {
            continue;
        }
        std::vector<int> add_indices = shortest_unselected_path_from_route(
            graph,
            selected,
            index,
            LOCAL_PRIZE_ACCESS_RESCUE_MAX_NEW_NODES
        );
        if (add_indices.empty() || add_indices.back() != index) {
            continue;
        }
        ...
        for (int add_index : add_indices) {
            cost += graph.nodes[add_index].cost;
            gain += route_node_score(graph.nodes[add_index], weights, &route_bonuses, add_index);
        }
        if (cost <= 0) {
            continue;
        }
        paths.push_back({std::move(add_indices), gain, cost});
    }

    std::sort(paths.begin(), paths.end(), [](const PrizeAccessPath& left, const PrizeAccessPath& right) {
        double left_ratio = left.heuristic_gain / static_cast<double>(std::max(left.cost, 1));
        double right_ratio = right.heuristic_gain / static_cast<double>(std::max(right.cost, 1));
        if (std::abs(left_ratio - right_ratio) > 1e-12) return left_ratio > right_ratio;
        if (std::abs(left.heuristic_gain - right.heuristic_gain) > 1e-12) return left.heuristic_gain > right.heuristic_gain;
        return left.cost < right.cost;
    });
    if (static_cast<int>(paths.size()) > LOCAL_PRIZE_ACCESS_RESCUE_MAX_PATHS) {
        paths.resize(LOCAL_PRIZE_ACCESS_RESCUE_MAX_PATHS);
    }
    return paths;
}

void collect_remove_combinations(
    const Graph& graph,
    ...
    if (static_cast<int>(combinations.size()) >= max_combinations) {
        return;
    }
    ...
    if (static_cast<int>(current.size()) >= LOCAL_PRIZE_ACCESS_RESCUE_MAX_REMOVES) {
        return;
    }
    for (int index = start; index < static_cast<int>(remove_candidates.size()); ++index) {
        int remove_index = remove_candidates[index];
        current.push_back(remove_index);
        collect_remove_combinations(
            graph,
            remove_candidates,
            index + 1,
            needed_cost,
            current,
            current_cost + graph.nodes[remove_index].cost,
            combinations,
            max_combinations
        );
        current.pop_back();
...
std::vector<double> score_patch_candidates_parallel(
    const Graph& graph,
    const std::vector<Glyph>& glyphs,
    const WeightModel& weights,
    const std::map<std::string, double>& starting_stats,
    const ScoringContext& context,
    const std::vector<unsigned char>& selected,
    const std::vector<RoutePatchCandidate>& candidates,
    int points_limit,
    int worker_count
) {
    ...
    auto score_one = [&](size_t index) {
        std::vector<unsigned char> trial = selected;
        for (int remove_index : candidates[index].remove_indices) {
            trial[remove_index] = 0;
        }
        for (int add_index : candidates[index].add_indices) {
            trial[add_index] = 1;
        }
        if (!route_connected_selection(graph, trial)) {
            return -std::numeric_limits<double>::infinity();
        }
        return route_score_value(graph, glyphs, weights, starting_stats, context, trial, points_limit);
    };

    if (active_workers == 1) {
        for (size_t index = 0; index < candidates.size(); ++index) {
            scores[index] = score_one(index);
        }
        return scores;
    }
...
std::vector<int> removable_route_nodes(
    const Graph& graph,
    const std::vector<unsigned char>& selected,
    const WeightModel& weights,
    int candidate_limit = LOCAL_IMPROVEMENT_REMOVE_CANDIDATES
) {
...
std::unordered_map<int, double> glyph_relocation_add_priorities(
    ...
    for (const GlyphEvaluation& evaluation : assigned_glyphs) {
        ...
        double desired_fill = glyph_threshold_hint_fill_target(info, weights, scaling_weight);
        bool useful_scaling = glyph_has_useful_threshold_scaling(info, scaling_weight);
        for (int node_index : radius_nodes_for(context, evaluation.socket_index, evaluation.glyph_index)) {
            if (selected[node_index]) {
                continue;
            }
            ...
            } else if (useful_scaling && weights.glyph_route.excess_factor > 0.0) {
                score += stat_value *
                    std::max(weight_for(weights.weights, info.threshold_stat), 0.0) *
                    weights.glyph_route.excess_factor;
            }
...
std::vector<double> selected_glyph_stat_pressure(
    ...
    for (const GlyphEvaluation& evaluation : assigned_glyphs) {
        if (evaluation.glyph_index < 0 || evaluation.glyph_index >= static_cast<int>(context.glyph_info.size())) {
            continue;
        }
        const GlyphInfo& info = context.glyph_info[evaluation.glyph_index];
        if (info.threshold_stat.empty()) {
        ...
        for (int node_index : radius_nodes_for(context, evaluation.socket_index, evaluation.glyph_index)) {
            ...
            if (stat_value > 0.0) {
                double after_remove = evaluation.stat_in_radius - stat_value;
                double protected_stat = std::clamp(info.requirement - after_remove, 0.0, stat_value);
                double excess_stat = std::max(stat_value - protected_stat, 0.0);
                pressure[node_index] += protected_stat + excess_stat * weights.glyph_route.excess_factor;
            }
        }
    }
    return pressure;
...
void improve_route_glyph_relocations(
    ...
    for (int pass = 0; pass < GLYPH_RELOCATION_MAX_PASSES; ++pass) {
        ...
        int max_add_cost = 0;
        std::vector<double> glyph_pressure = selected_glyph_stat_pressure(graph, weights, context, route.selected, assigned);
...
std::vector<int> threshold_excess_remove_nodes(
    const Graph& graph,
    const WeightModel& weights,
    const ScoringContext& context,
    const std::vector<unsigned char>& selected,
    ...
    std::vector<double> glyph_pressure = selected_glyph_stat_pressure(graph, weights, context, selected, assigned_glyphs);
    std::vector<int> candidates;
    for (int node_index = 0; node_index < static_cast<int>(selected.size()); ++node_index) {
        if (!selected[node_index] || node_index == graph.start_index || graph.nodes[node_index].cost <= 0) {
            continue;
        }
        const std::string& type = graph.nodes[node_index].type;
        if (type != "normal" && type != "magic") {
            continue;
        }
        if (!route_connected_without_node(graph, selected, node_index)) {
            continue;
        }

        bool threshold_contributor = false;
        bool removable_without_breaking_requirement = true;
        for (const GlyphEvaluation& evaluation : assigned_glyphs) {
            if (evaluation.glyph_index < 0 || evaluation.glyph_index >= static_cast<int>(context.glyph_info.size())) {
                continue;
            }
            const GlyphInfo& info = context.glyph_info[evaluation.glyph_index];
            if (info.threshold_stat.empty() || info.requirement <= 0.0 || !evaluation.requirement_met) {
                continue;
            }
            double stat_value = weight_for(graph.nodes[node_index].stats, info.threshold_stat);
            if (stat_value <= 0.0) {
            ...
            for (int radius_node : radius_nodes_for(context, evaluation.socket_index, evaluation.glyph_index)) {
                if (radius_node == node_index) {
                    in_radius = true;
                    break;
                }
            }
            ...
            threshold_contributor = true;
            if (evaluation.stat_in_radius - stat_value < info.requirement - 1e-9) {
                removable_without_breaking_requirement = false;
                break;
            }
        }
        if (threshold_contributor && removable_without_breaking_requirement) {
            candidates.push_back(node_index);
        }
    }

    std::sort(candidates.begin(), candidates.end(), [&](int left, int right) {
        double left_score = node_base_score(graph.nodes[left], weights) + glyph_pressure[left] * 3.0;
        double right_score = node_base_score(graph.nodes[right], weights) + glyph_pressure[right] * 3.0;
        if (std::abs(left_score - right_score) > 1e-12) return left_score < right_score;
        return graph.nodes[left].id < graph.nodes[right].id;
    });
    if (static_cast<int>(candidates.size()) > GLYPH_EXCESS_PRUNE_REMOVE_CANDIDATES) {
        candidates.resize(GLYPH_EXCESS_PRUNE_REMOVE_CANDIDATES);
    }
    return candidates;
}

void improve_route_threshold_excess_prune(
    const Graph& graph,
    const std::vector<Glyph>& glyphs,
    const WeightModel& weights,
    const std::map<std::string, double>& starting_stats,
    ...
    for (int pass = 0; pass < GLYPH_EXCESS_PRUNE_MAX_PASSES; ++pass) {
        std::vector<GlyphEvaluation> assigned = assign_glyphs(
            graph,
            glyphs,
            weights,
        ...
        std::vector<int> remove_candidates = threshold_excess_remove_nodes(
            graph,
            weights,
            context,
            route.selected,
        ...
        for (int remove_index : remove_candidates) {
            int remove_cost = graph.nodes[remove_index].cost;
            for (int add_index : add_candidates) {
                if (route.selected[add_index] || add_index == remove_index) {
                    continue;
                }
                int add_cost = graph.nodes[add_index].cost;
                if (current_points - remove_cost + add_cost > points_limit) {
...
bool apply_best_prize_access_rescue(
    const Graph& graph,
    const std::vector<Glyph>& glyphs,
    const WeightModel& weights,
    const std::map<std::string, double>& starting_stats,
    ...
    std::vector<PrizeAccessPath> access_paths = prize_access_paths(graph, route.selected, weights, route_bonuses);
    if (access_paths.empty()) {
        return false;
    }

    std::vector<int> remove_candidates = removable_route_nodes(graph, route.selected, weights);
    if (static_cast<int>(remove_candidates.size()) > LOCAL_PRIZE_ACCESS_RESCUE_REMOVE_CANDIDATES) {
        remove_candidates.resize(LOCAL_PRIZE_ACCESS_RESCUE_REMOVE_CANDIDATES);
    }

    int spare_points = points_limit - current_points;
    std::vector<RoutePatchCandidate> patch_candidates;
    patch_candidates.reserve(LOCAL_PRIZE_ACCESS_RESCUE_MAX_CANDIDATES);
    for (const PrizeAccessPath& path : access_paths) {
        int needed_remove_cost = std::max(0, path.cost - spare_points);
        std::vector<std::vector<int>> remove_combinations;
        std::vector<int> current;
        collect_remove_combinations(
            graph,
            remove_candidates,
            ...
            0,
            remove_combinations,
            LOCAL_PRIZE_ACCESS_RESCUE_MAX_REMOVE_COMBINATIONS_PER_PATH
        );
        for (const std::vector<int>& remove_indices : remove_combinations) {
            int remove_cost = 0;
            for (int remove_index : remove_indices) {
                remove_cost += graph.nodes[remove_index].cost;
            }
            if (current_points - remove_cost + path.cost > points_limit) {
                continue;
            }
            RoutePatchCandidate candidate;
            candidate.remove_indices = remove_indices;
            candidate.add_indices = path.add_indices;
            patch_candidates.push_back(std::move(candidate));
            if (static_cast<int>(patch_candidates.size()) >= LOCAL_PRIZE_ACCESS_RESCUE_MAX_CANDIDATES) {
                break;
            }
        }
        if (static_cast<int>(patch_candidates.size()) >= LOCAL_PRIZE_ACCESS_RESCUE_MAX_CANDIDATES) {
    ...
    int add_cost = 0;
    for (int add_index : best.add_indices) {
        add_cost += graph.nodes[add_index].cost;
    }
...
void append_unique_indices(std::vector<int>& target, const std::vector<int>& source, int max_size = 0) {
    for (int index : source) {
        if (max_size > 0 && static_cast<int>(target.size()) >= max_size) {
            return;
        }
...
void improve_route_final_single_swaps(
    ...
    for (int pass = 0; pass < FINAL_SINGLE_SWAP_MAX_PASSES; ++pass) {
        std::vector<GlyphEvaluation> assigned = assign_glyphs(
            graph,
            glyphs,
            weights,
        ...
        std::vector<int> remove_candidates = glyph_relocation_remove_nodes(
            graph,
            route.selected,
            weights,
            glyph_pressure
        );
        append_unique_indices(
            remove_candidates,
            threshold_excess_remove_nodes(graph, weights, context, route.selected, assigned),
            FINAL_SINGLE_SWAP_REMOVE_CANDIDATES
        );
        append_unique_indices(
            remove_candidates,
            removable_route_nodes(graph, route.selected, weights, FINAL_SINGLE_SWAP_REMOVE_CANDIDATES),
            FINAL_SINGLE_SWAP_REMOVE_CANDIDATES
        );
        if (remove_candidates.empty()) {
            break;
        ...
        append_unique_indices(
            add_candidates,
            adjacent_add_nodes(graph, route.selected, weights, route_bonuses, max_add_cost),
            FINAL_SINGLE_SWAP_ADD_CANDIDATES
        );
        if (add_candidates.empty()) {
            break;
        }
        ...
        std::vector<SwapCandidate> swap_candidates;
        swap_candidates.reserve(remove_candidates.size() * add_candidates.size());
        for (int remove_index : remove_candidates) {
            int remove_cost = graph.nodes[remove_index].cost;
            for (int add_index : add_candidates) {
                if (route.selected[add_index] || add_index == remove_index) {
...
RouteOutput improve_route_locally(
    ...
    for (int pass = 0; pass < LOCAL_IMPROVEMENT_MAX_PASSES; ++pass) {
        ...
        std::vector<int> add_candidates = adjacent_add_nodes(graph, route.selected, weights, route_bonuses, max_add_cost);
        if (add_candidates.empty()) {
            if (apply_best_prize_access_rescue(
                    graph,
                    glyphs,
                    weights,
                    starting_stats,
    ...
    improve_route_final_single_swaps(
        graph,
        glyphs,
        weights,
        starting_stats,
