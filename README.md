...
* `--local-repair-top-k`: Сколько лучших pre-local маршрутов отправлять в дорогой локальный ремонт. По умолчанию `3`; профильное поле `local_repair_top_k` делает то же самое. Большие значения повышают шанс найти лучший post-local результат, но почти линейно увеличивают стоимость локального ремонта.
