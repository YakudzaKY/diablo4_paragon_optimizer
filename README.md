...
- **`weights`** — веса статов. Каждый стат узлов × вес → суммируется.
  - `glyph_bonus` — сильный бонус **за каждый активированный глиф** (порог набран).
  - `glyph_socket` — помогает эвристике приоритетно добираться до сокетов.
- **`glyphs`** — приоритет конкретных глифов (по id из схемы). Неактивированный глиф получает только **25%** веса.
- **`scheme`**:
...
**Скор маршрута** = (статы × веса) + (типы узлов: normal 0.1 / magic 0.2 / rare 0.3 / legendary 0.4 + overrides) + (бонусы активированных редких/легендарных) + (скор глифов) − штрафы.

`glyph_route` влияет **только** на эвристику построения маршрутов, не на финальную оценку.
