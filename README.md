...
`glyph_route` не добавляет постоянный стат в результат. Это эвристика маршрутизации: она временно повышает ценность нод в радиусе, которые помогают добрать threshold или следующие шаги `For every 5 ...`. Финальный вклад всё равно попадает в `glyph_score`, а не в `stats` выбранной ноды.
