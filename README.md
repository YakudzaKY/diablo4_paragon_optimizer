...
**todo**:
- English readme
- Output the HTML report in English if the system locale is not RU 
...
`glyph_levels` задаёт реальные уровни прокачанных глифов по id из `data/normalized/glyphs/<class>/*.json`. Если блок `glyph_levels` задан в профиле, он также является строгим списком глифов, которые оптимизатор имеет право ставить в сокеты; остальные глифы класса не участвуют в поиске. Допустимый диапазон — `1..max_level`; неизвестный id или уровень вне диапазона считается ошибкой профиля. Если блок `glyph_levels` отсутствует целиком, CLI сохраняет совместимое поведение и считает все глифы класса доступными с уровнем `1`. Для текущих билдов в примерах используется `51`: на текущем датасете это уже открывает оба апгрейда радиуса из `radius.upgrade_levels` и даёт радиус `5`.
...
**Важно про ограничения глифов:** поле `glyphs` в файле весов является только приоритетом, а не строгим whitelist. Строгий набор разрешенных глифов задается в профиле через `glyph_levels`: если там перечислены глифы, только они могут попасть в результат.
...
* `--profile`: Путь к JSON профилю запуска из `profiles/*.json`. Профиль может задавать `class`, `points`, `weights`, `glyph_levels`, `starting_stats`, `max_routes`, `candidate_targets`, `local_repair_top_k`, `workers`, `scheme`, `include_route_steps`, `no_html` и `data`.
...
* `--local-repair-top-k`: Сколько лучших pre-local маршрутов отправлять в дорогой локальный ремонт. По умолчанию `4`; профильное поле `local_repair_top_k` делает то же самое. Большие значения повышают шанс найти лучший post-local результат, но почти линейно увеличивают стоимость локального ремонта.
