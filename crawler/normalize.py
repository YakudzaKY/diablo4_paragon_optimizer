...
def parse_stats_from_search_text(search_text: str) -> dict[str, float]:
    ...
    simple_patterns = [
        ...
        (r"bleed damage\s+magic\s+node\s+\+?(\d+(?:\.\d+)?)%(?!.*to bleeding)", "bleed_damage"),
        ...
        (r"minion damage\s+magic\s+node\s+\+?(\d+(?:\.\d+)?)%(?!.*(?:skeleton mage|skeleton warrior|golem))", "minion_damage"),
        ...
        (r"trap damage\s+magic\s+node\s+\+?(\d+(?:\.\d+)?)%(?!.*to enemies affected)", "trap_damage"),
        ...
        (r"burning damage\s+magic\s+node\s+\+?(\d+(?:\.\d+)?)%(?!.*to enemies)", "burning_damage"),
